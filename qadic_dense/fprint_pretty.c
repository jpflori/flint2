...
int _qadic_dense_fprint_pretty(FILE * file, const fmpz *u, long len, long v, 
    const qadic_dense_ctx_t ctx)
{
    ...
    else if ((&ctx->pctx)->mode == PADIC_SERIES)
    {
        ...
        if (i < len)
        {
            printf("ERROR (qadic_dense_fprint_pretty).  u < 0 in SERIES mode.\n");
            abort();
        }
    ...
    {
        printf("Exception (qadic_dense_fprint_pretty).  Unknown print mode.\n");
        abort();
    }
...
{
    return _qadic_dense_fprint_pretty(file, op->coeffs, op->length, op->val, ctx);
}
