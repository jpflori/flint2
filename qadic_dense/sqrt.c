...
/*
    ...
    finite fields and Hensel lifting.
        ...
        - _qadic_dense_sqrtmod_p()
        - _qadic_dense_sqrtmod_2()
        - _qadic_dense_sqrt_p()
        - _qadic_dense_sqrt()
        - qadic_dense_sqrt()
 */
                             ...
                             const fmpz_t p)
{
    ...
    for (i = d; i > 0; )
    {
        if (i == d)
        {
            ...
            _qadic_dense_pow(w, rop, d, z, mod, invmod, lenmod, p);
 ...
 */
int 
_artin_schreier_preimage_dense(fmpz *rop, const fmpz *op, long len, 
                         const fmpz *mod, const fmpz *invmod, long lenmod)
{
    const long d   = lenmod - 1;
    const fmpz_t p = {2L};
    ...
    for (i = 0; i < d; i++)
    {
        ...
        _fmpz_poly_sqr(g, e, i+1);
        _qadic_dense_reduce_no_mod(f, g, 2*(i+1)-1, mod, invmod, lenmod);
                 ...
                 const fmpz_t p)
{
    ...
    else if (fmpz_fdiv_ui(p, 8) == 5 && (d & 1L))
    {
        ...
        _qadic_dense_pow(v, op, len, g, mod, invmod, lenmod, p);

        _fmpz_poly_mul(r, v, d, op, len);
        _qadic_dense_reduce(w, r, d + len - 1, mod, invmod, lenmod, p);

        _fmpz_poly_mul(r, v, d, w, d);
        _qadic_dense_reduce(y, r, 2 * d - 1, mod, invmod, lenmod, p);
    ...
    else
    {
        ...
        /* Find a non-residue g */
        _find_nonresidue(g, mod, invmod, lenmod, p);
 ...
 */
static int 
_qadic_dense_sqrt_2(fmpz *rop, const fmpz *op, long len, 
              const fmpz *mod, const fmpz *invmod, long lenmod, long N)
{
    ...
    _fmpz_mod_poly_invmod(r, t, len, mod, lenmod, p8);   /* 1/u mod 8          */
...
printf("r = "), _fmpz_vec_print(r, d), printf("\n");
    ...
    _qadic_dense_sqrtmod_2(s, t, d, mod, invmod, lenmod);    /* invsqrt(u) mod 2   */
printf("t = "), _fmpz_vec_print(t, d), printf("\n");
printf("s = "), _fmpz_vec_print(s, d), printf("\n");

    _fmpz_poly_sqr(t, s, d);
    _qadic_dense_reduce_no_mod(c, t, 2 * d - 1, mod, invmod, lenmod);
    ...
    _fmpz_poly_sqr(t, s, d);
    _qadic_dense_reduce(g, t, 2 * d - 1, mod, invmod, lenmod, p);
    _fmpz_mod_poly_invmod(t, g, d, mod, lenmod, p);
    _fmpz_poly_mul(r, c, d, t, d);
    _qadic_dense_reduce(g, r, 2 * d - 1, mod, invmod, lenmod, p);

    ans = _artin_schreier_preimage_dense(r, g, d, mod, invmod, lenmod);

    _fmpz_vec_zero(rop, 2 * d - 1);

    if (ans)
    {
        _fmpz_poly_mul(c, r, d, s, d);
        _qadic_dense_reduce(t, c, 2 * d - 1, mod, invmod, lenmod, p);
        ...
        if (N == 1)
        {
            ...
            _fmpz_mod_poly_invmod(rop, s, d, mod, lenmod, p);
        }
        else if (N == 2)
        {
            _fmpz_mod_poly_invmod(rop, s, d, mod, lenmod, p4);
        }
        else  /* N >= 3 */
        {
            long *e, i, n;
            ...
            {
                _fmpz_poly_sqr(c, rop, d);
                _qadic_dense_reduce_no_mod(s, c, 2 * d - 1, mod, invmod, lenmod);
                _fmpz_poly_mul(c, s, d, u + i * len, len);
                _qadic_dense_reduce_no_mod(t, c, d + len - 1, mod, invmod, lenmod);
                fmpz_sub_ui(t, t, 1);
                _fmpz_vec_scalar_fdiv_r_2exp(t, t, d, 1);
                _fmpz_poly_mul(c, t, d, rop, d);
                _qadic_dense_reduce_no_mod(s, c, 2 * d - 1, mod, invmod, lenmod);
                _fmpz_poly_sub(rop, rop, d, s, d);
                _fmpz_vec_scalar_fdiv_r_2exp(rop, rop, d, e[i]);
            }
            {
                _fmpz_poly_mul(c, rop, d, u + 1 * len, len);
                _qadic_dense_reduce_no_mod(s, c, d + len - 1, mod, invmod, lenmod);
                _fmpz_poly_sqr(c, s, d);
                _qadic_dense_reduce_no_mod(t, c, 2 * d - 1, mod, invmod, lenmod);
                _fmpz_poly_sub(t, u + 0 * len, len, t, d);
                _fmpz_vec_scalar_fdiv_q_2exp(t, t, d, 1);
                _fmpz_poly_mul(c, rop, d, t, d);
                _qadic_dense_reduce_no_mod(r, c, 2 * d - 1, mod, invmod, lenmod);
                ...
                const fmpz_t p, long N)
{
    if (*p == 2L)
    {
        return _qadic_dense_sqrt_2(rop, op, len, mod, invmod, lenmod, N);
    }
    else
    {
