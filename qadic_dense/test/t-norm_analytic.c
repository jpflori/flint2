...
main(void)
{
    ...
    for (i = 0; i < 2000; i++)
    {
        ...
        qadic_dense_randtest_val(a, state, fmpz_cmp_ui(p, 2) == 0 ? 2 : 1, ctx);
        qadic_dense_reduce(a, ctx);
        qadic_dense_one(b, ctx);
        qadic_dense_add(a, a, b, ctx);

        qadic_dense_norm_analytic(x, a, ctx);

        qadic_dense_one(b, ctx);
        for (j = 0; j < d; j++)
        {
            qadic_dense_frobenius(c, a, j, ctx);
            qadic_dense_mul(b, b, c, ctx);
        }
