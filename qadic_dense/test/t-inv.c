...
main(void)
{
    ...
    for (i = 0; i < 2000; i++)
    {
        ...
        qadic_dense_inv(c, b, ctx);
        qadic_dense_inv(b, b, ctx);
    ...
    for (i = 0; i < 2000; i++)
    {
        ...
        do qadic_dense_randtest_val(a, state, 0, ctx);
        while (qadic_dense_is_zero(a));

        qadic_dense_inv(b, a, ctx);
        qadic_dense_mul(b, a, b, ctx);

        result = (qadic_dense_is_one(b, ctx));
        if (!result)
        {
            printf("FAIL:\n\n");
            printf("a = "), qadic_dense_print_pretty(a, ctx), printf("\n");
